...
pub struct RepeatedTokens<
    X: 'static,
    F1: FnMut(&mut VecDeque<Token>) -> Result<Option<X>, SyntaxError>,
    F2: FnMut(&mut VecDeque<Token>) -> Result<bool, SyntaxError>,
> {
    pattern_ingest: F1,
    separator_ingest: F2,
}
...
{
    pub fn ingest_all(&mut self, tokens: &mut VecDeque<Token>) -> Result<Vec<X>, SyntaxError> {
...
pub fn repeated_tokens<X, F1: FnMut(&mut VecDeque<Token>) -> Result<Option<X>, SyntaxError>>(
    ingestor: F1,
    separator: Token,
) -> RepeatedTokens<X, F1, impl FnMut(&mut VecDeque<Token>) -> Result<bool, SyntaxError>> {
    RepeatedTokens {
