...
pub struct IntersectionStatement {
    ...
    ops: Vec<PseudoOp>,
    items: Option<HashSet<IteratorItem>>,
    original_order: Option<VecDeque<IteratorItem>>,
    init_needed: bool,
}
...
impl Iterator for IntersectionStatement {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.ops.len() == 0 {
            ...
            let original_order: VecDeque<IteratorItem> = real_op.collect();
            let mut set: HashSet<IteratorItem> =
            ...
            if self.ops.len() != 1 && !set.is_empty() {
                for i in 1..self.ops.len() {
                    ...
                    real_op.enter(self.context.take().unwrap());
                    let set2: HashSet<IteratorItem> = real_op.collect();
                    self.context = Some(real_op.escape());
...
impl Op for IntersectionStatement {
...
impl FunctionFactory<IntersectionStatement> for IntersectionFunctionFactory {
...
pub type IntersectionStatementFactory =
    FunctionStatementFactory<IntersectionStatement, IntersectionFunctionFactory>;
